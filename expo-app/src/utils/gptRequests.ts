...
export const generateItinerary = async (prompt: string) => {
  try {
    const response = await fetch(`https://guia-turistico-alpha.vercel.app//:3000/api/generateItinerary`, {
      method: 'POST',
      headers: {
...
export const generateChatAnswers = async (prompt: string) => {
  try {
    const response = await fetch(`https://guia-turistico-alpha.vercel.app//:3000/api/justchat`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
